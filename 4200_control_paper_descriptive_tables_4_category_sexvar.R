...
colnames(mixmat.3cat) <- c("From", colnames(mixingmatrix(baseline.full.network.with.missing[[1]],'sex.var')$mat) )

rowcount <- 1
for (i in 1:6){
  mixmat.3cat[rowcount,1] <- paste0("School ",i)
  mixmat.3cat[(rowcount + 1): (rowcount + 3) ,2:4] <- mixingmatrix(baseline.full.network.with.missing[[i]],'sex.var')$mat 
  mixmat.3cat[(rowcount + 1): (rowcount + 3) ,1]   <- rownames(mixingmatrix(baseline.full.network.with.missing[[i]],'sex.var')$mat )
  rowcount <- rowcount + 4
}
...
mixmat.4cat <- matrix(NA,nc = 5, nr = 30)

colnames(mixmat.4cat) <- c("From", colnames(mixingmatrix(baseline.full.network.with.missing[[1]],'talk.var')$mat) )

rowcount <- 1
for (i in 1:6){
  mixmat.4cat[rowcount,1] <- paste0("School ",i)
  mixmat.4cat[(rowcount + 1): (rowcount + 4) ,2:5] <- mixingmatrix(baseline.full.network.with.missing[[i]],'talk.var')$mat 
  mixmat.4cat[(rowcount + 1): (rowcount + 4) ,1]   <- rownames(mixingmatrix(baseline.full.network.with.missing[[i]],'talk.var')$mat )
  rowcount <- rowcount + 5
}
...
colnames(mixmat.4cat) <- c("From", colnames(mixingmatrix(control.full.network.with.missing[[1]],'talk.var')$mat) )

rowcount <- 1
for (i in 1:6){
  mixmat.4cat[rowcount,1] <- paste0("School ",i)
  mixmat.4cat[(rowcount + 1): (rowcount + 4) ,2:5] <- mixingmatrix(control.full.network.with.missing[[i]],'talk.var')$mat 
  mixmat.4cat[(rowcount + 1): (rowcount + 4) ,1]   <- rownames(mixingmatrix(control.full.network.with.missing[[i]],'talk.var')$mat )
  rowcount <- rowcount + 5
}
