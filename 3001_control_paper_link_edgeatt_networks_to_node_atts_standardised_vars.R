...
table(baseline.attributes$know.var, baseline.attributes$std.know, useNA = "ifany")
...
table(get.vertex.attribute(control.edge.att.network[[6]], "talk.var") )
...
control.edge.att.network[[i]] %v% "scale.var" <- link.atts$scale.var[match(network.vertex.names(control.edge.att.network[[i]]), link.atts$id)]
...
control.edge.att.network[[i]] %v% "std.know" <- link.atts$std.know[match(network.vertex.names(control.edge.att.network[[i]]), link.atts$id)]
control.edge.att.network[[i]] %v% "std.conf" <- link.atts$std.conf[match(network.vertex.names(control.edge.att.network[[i]]), link.atts$id)]
control.edge.att.network[[i]] %v% "std.att"  <- link.atts$std.att[match(network.vertex.names(control.edge.att.network[[i]]), link.atts$id)]
control.edge.att.network[[i]] %v% "know.var" <- link.atts$know.var[match(network.vertex.names(control.edge.att.network[[i]]), link.atts$id)]
control.edge.att.network[[i]] %v% "conf.var" <- link.atts$conf.var[match(network.vertex.names(control.edge.att.network[[i]]), link.atts$id)]
control.edge.att.network[[i]] %v% "att.var"  <- link.atts$att.var[match(network.vertex.names(control.edge.att.network[[i]]), link.atts$id)]
control.edge.att.network[[i]] %v% "school.id" <- link.atts$respondent_school[match(network.vertex.names(control.edge.att.network[[i]]), link.atts$id)]

control.edge.att.network[[i]] %v% "outschool" <- as.character(
  as.numeric(control.outside.school[[i]]$outschfriends[match(network.vertex.names(control.edge.att.network[[i]]), control.outside.school[[i]]$respondent_id)]) )
...
table(get.vertex.attribute(control.full.network.with.missing[[6]], "std.know") , get.vertex.attribute(control.full.network.with.missing[[6]], "know.var") )
#####            #####
#####   Baseline #####
#####            #####
#####            #####
...
colnames(baseline.net.dataset[[i]]) <- c( "id", "school.id", "gender",
                                          ...
                                          'att.var','conf.var','scale.var','sex.var','talk.var',
...
#######Somethig wrong here **** table(baseline.net.dataset[[6]]$std.know , baseline.net.dataset[[6]]$know.var )


save(control.net.dataset, file = "//192.168.0.17/stash_sna/Data/AnonymisedData/working data/control_net_dataset_3001.rdata")
save(baseline.net.dataset, file = "//192.168.0.17/stash_sna/Data/AnonymisedData/working data/baseline_net_dataset_3001.rdata")
