...
###Change the  raw'know.var' to standardised 'std.know' and same for other variables 
file[[73]] <- "                                      + absdiff(\"std.know\")"
file[[74]] <- "                                      + absdiff(\"std.att\")"
file[[75]] <- "                                      + absdiff(\"std.conf\")"

file[[78]] <- "                                      + nodeicov(\"std.know\")"
file[[79]] <- "                                      + nodeicov(\"std.att\")"
file[[80]] <- "                                      + nodeicov(\"std.conf\")"

file[[81]] <- "                                      + nodeocov(\"std.know\")"
file[[82]] <- "                                      + nodeocov(\"std.att\")"
file[[83]] <- "                                      + nodeocov(\"std.conf\")"

file[[97]] <- "    save(imputation.list, file = paste0(\"ergm_6115_imp\",num_iters,\"_sch_\",sch,\"_\",savename,\".rdata\"))"
...
#loop over school and imputation and save file
    ...
    for (sch in 1:6){      
      for (imp in 1:20){
        ...
        writeLines(file[1:140], con = paste0("6115_i",imp,"_s",sch,".R"))
    }
  }
...
for (sch in 1:6){      
  for (imp in 1:20){
    ...
    infile[11] <- paste0(" R CMD BATCH /export/home/mmc78h/STASH/6115_i",imp,"_s",sch,".R /export/home/mmc78h/STASH/6115_i",imp,"_s",sch,".out")
    setwd("//192.168.0.17/stash_sna/Data/AnonymisedData/working data/hpc_script_prep_may_2020")
    writeLines(infile, con = paste0("6115_i",imp,"_s",sch,".sh"))
  }
}
  ...
  ####Change to run on centos7
    for (sch in 1:6){      
      for (imp in 1:20){
        infile <- readLines(paste0("6115_i",imp,"_s",sch,".sh" ))
...
for (sch in 1:6){      
  for (imp in 1:20){
    ...
    pastefile[counter] <- paste0("qsub 6115_i",imp,"_s",sch,".sh")
    counter <- counter + 1
  }
}

writeLines(pastefile, con = "6115_HPC_copypaste_commands_into_putty.txt")
